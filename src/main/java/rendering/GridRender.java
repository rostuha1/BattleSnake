...
public class GridRender implements Render {
    ...
    public void render() {
        ...
        for (int i = 0; i < SQUARE_NUMBER + 1; i++) {
            glVertex2i(startPoint + parts[i], 0);
            glVertex2i(startPoint + parts[i], WindowManager.getHeight());
        }

        for (int i = 0; i < SQUARE_NUMBER + 1; i++) {
            glVertex2i(startPoint, parts[i]);
            glVertex2i(startPoint + WindowManager.getHeight(), parts[i]);
        }
