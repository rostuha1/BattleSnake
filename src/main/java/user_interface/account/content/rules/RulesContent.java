...
public class RulesContent extends BorderPane {
    ...
    private final TextArea battleRules = new TextArea("" +
            ...
            "У випадку перемоги змія отримує +10 балів до рейтингу, а у випадку поразки -5 балів.\n" +
            "При достроковому завершенні гри рейтинг все-одно змінюється.");
    ...
    private final TextArea intelligenceRules = new TextArea("" +
            ...
            "бар'єр), які позначатимуть певний елемент ігрового поля. Кожна карта по черзі з усіх сторін\n" +
            "порівнюється з частиною ігрового поля і, у випадку повного співпадіння, виконується, \n" +
            "тобто змія робить крок. Приорітети карток зменшуються у порядку їх розташування (перша \n" +
            "карта має найвищий приорітет). У випадку, коли елементи жодної з карт не співпали з елементами\n " +
            "ігрового поля, змія ходить випадковим чином.\n" +
            ...
            "- EXCEPT - елемент з цією властивістю буде вважатися таким, що співпав, якщо на його місці\n" +
            "виявиться будь-який елемент, крім вказаного.");
